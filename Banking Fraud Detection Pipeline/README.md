...
During the project, I faced and solved multiple real-world data engineering issues:

| Error / Challenge | What I Learned |
|---|---|
| Ambiguous column name `BATCH_ID` | Use table aliases when the same column exists in multiple CTEs or tables |
| Snowflake procedure failed from Airflow | Test each stored procedure manually before orchestration |
| Stored procedure syntax errors | Follow the correct Snowflake `CREATE PROCEDURE` structure |
| Pipeline was processing old data again | Use `batch_id` and `load_time` to process only the latest batch |
| Duplicate records in raw data | Use `ROW_NUMBER()` to separate valid and duplicate records |
| Invalid timestamps and amounts | Use `TRY_TO_TIMESTAMP` and `TRY_CAST` for safe data validation |
| Inconsistent login status values | Standardize raw values before loading curated tables |
| Unknown device detection confusion | Maintain a known-device reference table |
| Impossible travel detection | Use geospatial distance and time-difference logic |
| SCD Type 2 join issue | Compare incoming records only with active customer records |
| Airflow task dependency order | Run raw loading first, then curated loads, then fraud checks |

A detailed version is available in [`docs/ERRORS_AND_LEARNINGS.md`](docs/ERRORS_AND_LEARNINGS.md).